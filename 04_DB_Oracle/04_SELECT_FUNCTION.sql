...
SELECT
    ...
    RTRIM('65165466KH615616','0123456789') -- 오른쪽에서 해당하는 숫자들 제거
FROM DUAL;
...
*/
SELECT 
    TRIM(LEADING 'Z' FROM 'ZZZZZKHZZZZ'), -- LTRIM
    TRIM(TRAILING 'Z' FROM 'ZZZZZKHZZZZ'), -- RTRIM
    TRIM(BOTH 'Z' FROM 'ZZZZZKHZZZZ'), -- 양쪽 모두 제거
    TRIM('        KH           ')
FROM DUAL;
...
SELECT 
    EMP_NO,
    RPAD(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') + 1), LENGTH(EMP_NO), '*'),
...
/*
    ADD_MONTHS(날짜, 숫자)
    - 특정 날짜의 입력받는 숫자만큼 개월 수를 더한 날짜 반환
*/
...
SELECT TO_DATE(20250604164232, 'YYYY-MM-DD HH24:MI:SS') FROM DUAL;
...
/*
    ...
    NVL/COALESCE(값1, 값2)
    - 값1이 NULL이 아니면 값1 반호나하고, 값1이 NULL이면 값2 반환
*/
...
-- 직급 코드가 J7인 사원을 급여를 10% 인상, 직급 코드가 J6인 사원은 급여는 15% 인상, 
-- 직급 코드가 J5인 사람은 급여를 20% 인상, 그 외 직급 사원은 급여를 5% 인상
...
-- 급여가 500만원 초과일 경우 1등급, 급여가 500만원 이하 350만원 초과일 경우 2등급
-- 급여가 350만원 이하 200만원 초과일 경우 3등급, 그 외의 경우 4등급
SELECT
    SALARY,
...
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY ROLLUP(JOB_CODE); -- 전체 합계 추산
...
/*
    ...
    - 여러 개의 쿼리문에서 조회하려고 하는 컬럼의 개수와 이름이 같아야 집합 연사자를 사용할 수 있음
...
-- 위 커리문 대신 WHERE절에서 OR 연산자 사용해도 처리가 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
-- 위 커리문 대신 WHERE절에서 AND 연산자 사용해도 처리가 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' AND SALARY > 3000000;

-- 4.MINUS
-- 부서코드가 'D5'면서 급여가 300만원 초과인 사원들 제외
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' 
MINUS
