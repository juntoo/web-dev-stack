/*
    ...
    - 관계형(Ralational) 데이터베이스
        ...
        1. DML(Data Manipulation Language) : 데이터 조작어
            - 데이터를 조작하기 위한 언어(INSERT : 추가, SLECET : 조회, UPDATE : 수정, DELETE : 삭제)
...
SELECT NAME || '의 MTBI는 ' || MBTI || '입니다' FROM USER_INFO;
...
--WHERE MBTI LIKE '%E%'AND MBTI LIKE '%T%';
...
-- 직급코드(JOB_CODE)가 J7이거나 J2인 사원들 중 급여가 200만원 이상인 사원들 조회(IN/OR사용)
SELECT *
FROM EMPLOYEE
...
-- MBTI는 오름차순, 나이는 내림차순 정렬(MBTI가 I인 분들 조회)
SELECT NAME, AGE, GENDER, MBTI
FROM USER_INFO
--WHERE MBTI LIKE 'I___'
...
FROM EMPLOYEE
--ORDER BY BONUS NULLS FIRST; -- 오름차순인 경우 NULL이 맨 뒤에
ORDER BY BONUS DESC NULLS LAST; -- 내림차순인 경우 NULL이 맨 앞에
...
-- USER_INFO에서 MBTI별 평균 나이를 계산하는데 평균나이가 30 이하인 MBTI만 조회
-- 단, 나이가 100살 이상인 경우는 제외
SELECT
    MBTI, AVG(CASE WHEN AGE < 100 THEN AGE END)
FROM USER_INFO
GROUP BY MBTI
