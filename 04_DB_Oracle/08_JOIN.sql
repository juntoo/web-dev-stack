...
SELECT E.EMP_NO 사원사번, E.EMP_NAME 사원명, E.DEPT_CODE 사원부서코드, E.MANAGER_ID 사수사번, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드
FROM EMPLOYEE E
...
SELECT NVL(DEPT_TITLE, '부서없음') 부서, TO_CHAR(ROUND(AVG(SALARY)), '999,999,999') 평균급여
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE;
...
-- 13. 사용자별 리뷰 수 조회
SELECT USER_ID, NAME, COUNT(REVIEW_ID)
FROM USER_INFO
LEFT JOIN REVIEW USING(USER_ID)
GROUP BY USER_ID, NAME;

-- 14. 봉준호 감독의 영화에 출연한 배우 조회
SELECT DIRECTOR, TITLE, NAME
FROM MOVIE_ACTOR
JOIN MOVIE USING(MOVIE_ID)
JOIN ACTOR USING(ACTOR_ID)
WHERE DIRECTOR = '봉준호';
