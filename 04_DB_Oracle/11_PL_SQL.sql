...
BEGIN
    LOOP
        ...
        LOOP 
            DBMS_OUTPUT.PUT_LINE(NUM || ' X ' || MULTIPLE || ' = ' || NUM*MULTIPLE);
            MULTIPLE := MULTIPLE + 1;
            IF MULTIPLE > 9 THEN  MULTIPLE := 1; EXIT; END IF;
        END LOOP;
        
        NUM := NUM + 1;
        IF NUM > 9 THEN EXIT; END IF;
    END LOOP;
END;
/
...
EXCEPTION
    WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나누기 연산기 0으로 나눌 수 없습니다');
END;
/
