...
public class C_For {
	 ...
	 * 초기값 대입 - 조건식 : 결과가 true이면 실행코드를 실행, false면 실행하지 않고 반복문을 빠져나감 - 증감식 : 반복문에서
	 * 사용하는 변수의 값을 증감, 증로 증감 연산자 사용
	 */
	// 1~5 출력
	public void method1() {
		for (int i = 1; i <= 5; i++) {
	...
	public void method3() {
		...
		/*
		 * continue 문 - continue 문은 반복문 안에서 사용 - 반복문 안에서 continue를 만나면 "현재 구문" 종료 - 반복문을
...
//		int random = (int) (Math.random() * 9 + 2); // 0.0 <= random < 1.0
		...
		int rand = random2.nextInt(9) + 2; // 0~8까지 
		int sum = 0;
		...
		System.out.println("1~랜덤값 합계 : " + sum);
	}
